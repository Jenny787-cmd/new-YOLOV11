...
from ultralytics.nn.modules import DClusConv, C3k2_DClus
...
class BaseModel(torch.nn.Module):
    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            for m in self.model.modules():
                if isinstance(m, (ultralytics.nn.modules.Conv, ultralytics.nn.modules.Conv2, ultralytics.nn.modules.DWConv)) and hasattr(m, "bn"):
                    if isinstance(m, ultralytics.nn.modules.Conv2):
                        m.fuse_convs()
                    m.conv = fuse_conv_and_bn(m.conv, m.bn)  # update conv
                    delattr(m, "bn")  # remove batchnorm
                    m.forward = m.forward_fuse  # update forward
                if isinstance(m, ultralytics.nn.modules.ConvTranspose) and hasattr(m, "bn"):
                    m.conv_transpose = fuse_deconv_and_bn(m.conv_transpose, m.bn)
                    delattr(m, "bn")  # remove batchnorm
                    m.forward = m.forward_fuse  # update forward
                if isinstance(m, ultralytics.nn.modules.RepConv):
                    m.fuse_convs()
                    m.forward = m.forward_fuse  # update forward
                if isinstance(m, ultralytics.nn.modules.RepVGGDW):
                    m.fuse()
                    m.forward = m.forward_fuse
                if isinstance(m, ultralytics.nn.modules.v10Detect):
    ...
    def _apply(self, fn):
        ...
        if isinstance(
            m, ultralytics.nn.modules.Detect
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, ultralytics.nn.modules.Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            s = 256  # 2x min stride
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (ultralytics.nn.modules.Segment, ultralytics.nn.modules.YOLOESegment, ultralytics.nn.modules.Pose, ultralytics.nn.modules.OBB)) else self.forward(x)
...
class ClassificationModel(BaseModel):
    ...
    def reshape_outputs(model, nc):
        ...
        if isinstance(m, ultralytics.nn.modules.Classify):  # YOLO Classify() head
...
class WorldModel(DetectionModel):
    ...
    def predict(self, x, profile=False, visualize=False, txt_feats=None, augment=False, embed=None):
        ...
        for m in self.model:  # except the head part
            ...
            if profile:
                self._profile_one_layer(m, x, dt)
            if isinstance(m, ultralytics.nn.modules.C2fAttn):
                x = m(x, txt_feats)
            elif isinstance(m, ultralytics.nn.modules.WorldDetect):
                x = m(x, ori_txt_feats)
            elif isinstance(m, ultralytics.nn.modules.ImagePoolingAttn):
...
class YOLOEModel(DetectionModel):
    ...
    def get_text_pe(self, text, batch=80, cache_clip_model=False, without_reprta=False):
        ...
        head = self.model[-1]
        assert isinstance(head, ultralytics.nn.modules.YOLOEDetect)
    ...
    def set_vocab(self, vocab, names):
        ...
        self.model[-1].lrpc = nn.ModuleList(
            ultralytics.nn.modules.LRPCHead(cls, pf[-1], loc[-1], enabled=i != 2)
    ...
    def predict(
        ...
        for m in self.model:  # except the head part
            ...
            if profile:
                self._profile_one_layer(m, x, dt)
            if isinstance(m, ultralytics.nn.modules.YOLOEDetect):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    if act:
        ultralytics.nn.modules.Conv.default_act = eval(act)  # redefine default activation, i.e. Conv.default_act = torch.nn.SiLU()
        if verbose:
    ...
    base_modules = frozenset(
        {
            ...
            ultralytics.nn.modules.C2fPSA,
            ultralytics.nn.modules.C2PSA,
            ...
            ultralytics.nn.modules.A2C2f,
            ultralytics.nn.modules.C3DClus
        }
    )
    repeat_modules = frozenset(  # modules with 'repeat' arguments
        {
            ...
            ultralytics.nn.modules.C2PSA,
            ultralytics.nn.modules.A2C2f,
            ultralytics.nn.modules.C3k2_DClus
        }
    )
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is ultralytics.nn.modules.C2fAttn:  # set 1) embed channels and 2) num heads
            ...
            if m in repeat_modules:
                ...
                n = 1
            if m is ultralytics.nn.modules.C3k2:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m is ultralytics.nn.modules.A2C2f:
                legacy = False
                if scale in "lx":  # for L/X sizes
                    args.extend((True, 1.2))
            if m is ultralytics.nn.modules.C2fCIB:
                legacy = False
        elif m is ultralytics.nn.modules.AIFI:
            args = [ch[f], *args]
        elif m in frozenset({ultralytics.nn.modules.HGStem, ultralytics.nn.modules.HGBlock}):
            c1, cm, c2 = ch[f], args[0], args[1]
            args = [c1, cm, c2, *args[2:]]
            if m is ultralytics.nn.modules.HGBlock:
                args.insert(4, n)  # number of repeats
                n = 1
        elif m is ultralytics.nn.modules.ResNetLayer:
            c2 = args[1] if args[3] else args[1] * 4
        elif m is torch.nn.BatchNorm2d:
            args = [ch[f]]
        elif m is ultralytics.nn.modules.Concat:
            c2 = sum(ch[x] for x in f)
        elif m in frozenset(
            {ultralytics.nn.modules.Detect, ultralytics.nn.modules.WorldDetect, ultralytics.nn.modules.YOLOEDetect, ultralytics.nn.modules.Segment, ultralytics.nn.modules.YOLOESegment, ultralytics.nn.modules.Pose, ultralytics.nn.modules.OBB, ultralytics.nn.modules.ImagePoolingAttn, ultralytics.nn.modules.v10Detect}
        ):
            args.append([ch[x] for x in f])
            if m is ultralytics.nn.modules.Segment or m is ultralytics.nn.modules.YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {ultralytics.nn.modules.Detect, ultralytics.nn.modules.YOLOEDetect, ultralytics.nn.modules.Segment, ultralytics.nn.modules.YOLOESegment, ultralytics.nn.modules.Pose, ultralytics.nn.modules.OBB}:
                m.legacy = legacy
        elif m is ultralytics.nn.modules.RTDETRDecoder:  # special case, channels arg must be passed in index 1
            args.insert(1, [ch[x] for x in f])
        elif m is ultralytics.nn.modules.CBLinear:
            c2 = args[0]
            c1 = ch[f]
            args = [c1, c2, *args[1:]]
        elif m is ultralytics.nn.modules.CBFuse:
            c2 = ch[f[-1]]
        elif m in frozenset({ultralytics.nn.modules.TorchVision, ultralytics.nn.modules.Index}):
            c2 = args[0]
            c1 = ch[f]
            args = [*args[1:]]
        else:
...
def guess_model_task(model):
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (ultralytics.nn.modules.Segment, ultralytics.nn.modules.YOLOESegment)):
                return "segment"
            elif isinstance(m, ultralytics.nn.modules.Classify):
                return "classify"
            elif isinstance(m, ultralytics.nn.modules.Pose):
                return "pose"
            elif isinstance(m, ultralytics.nn.modules.OBB):
                return "obb"
            elif isinstance(m, (ultralytics.nn.modules.Detect, ultralytics.nn.modules.WorldDetect, ultralytics.nn.modules.YOLOEDetect, ultralytics.nn.modules.v10Detect)):
                return "detect"
