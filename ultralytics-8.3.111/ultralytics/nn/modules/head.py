...
class Detect(nn.Module):
    ...
    # 支持训练和推理两种模式
    # 提供多种导出格式的兼容性
    dynamic = False  # force grid reconstruction是否强制重新计算网格
    export = False  # export mode# 是否处于导出模式
    format = None  # export format# 导出格式
    end2end = False  # end2end# 是否端到端模式
    max_det = 300  # max_det# 每张图像最大检测数,用于控制单张图像中最多输出的检测框数量
    shape = None
    anchors = torch.empty(0)  # init
    strides = torch.empty(0)  # init
    legacy = False  # backward compatibility for v3/v5/v8/v9 models# 是否使用旧版结构
    ...
    def forward_end2end(self, x):
        #端到端模式
        #在目标检测领域特指从原始图像输入到最终检测结果输出全部由单个模型完成
        """
